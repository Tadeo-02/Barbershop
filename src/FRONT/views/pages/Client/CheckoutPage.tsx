...
const CheckoutPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (!firstName && user.nombre) { setFirstName(user.nombre); didPrefill = true; }
        if (!lastName && user.apellido) { setLastName(user.apellido); didPrefill = true; }
        if (!phone && user.telefono) { setPhone(user.telefono); didPrefill = true; }
        if (!email && user.email) { setEmail(user.email); didPrefill = true; }
    ...
    const applyProfile = () => {
        if (!user) return;
        let didPrefill = false;
        if (user.nombre && firstName !== user.nombre) { setFirstName(user.nombre); didPrefill = true; }
        if (user.apellido && lastName !== user.apellido) { setLastName(user.apellido); didPrefill = true; }
        if (user.telefono && phone !== user.telefono) { setPhone(user.telefono); didPrefill = true; }
        if (user.email && email !== user.email) { setEmail(user.email); didPrefill = true; }
        if (user.codSucursal && branches.length > 0) {
            const found = branches.find((b) => b.codSucursal === user.codSucursal);
            if (found && selectedBranch !== found.codSucursal) { setSelectedBranch(found.codSucursal); didPrefill = true; }
        }
        if (didPrefill) setPrefilled(true);
    };
    ...
    const isProfileApplied = () => {
        ...
        const lastMatch = (!user.apellido && !lastName) || (user.apellido === lastName);
        const phoneMatch = (!user.telefono && !phone) || (user.telefono === phone);
        const emailMatch = (!user.email && !email) || (user.email === email);
        const branchMatch = !user.codSucursal || user.codSucursal === selectedBranch;
        return nameMatch && lastMatch && phoneMatch && emailMatch && branchMatch;
    };
    ...
    const handleConfirm = () => {
        ...
        const selectedBranchObj = branches.find((b) => b.codSucursal === selectedBranch) || null;
        ...
        navigate("/order-confirmation", {
            state: { orderId: orderNumber, pickupAt: pickupDate.toISOString(), order },
        });
    };
            ...
            <div className={styles.content}>
                <section className={styles.formSection}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                        <h3 style={{ margin: 0 }}>Datos del comprador</h3>
                        {isAuthenticated && (
                            <button
                                type="button"
                                ...
                                title={isProfileApplied() ? 'Datos ya coinciden con tu perfil' : 'Rellenar con datos del perfil'}
                            >
                    ...
                    {prefilled && (
                        <div className={styles.prefilledNote}>
                            Datos precargados desde tu perfil. Puedes editarlos antes de confirmar.
                        </div>
                    )}
                    <label>Nombre</label>
                    <input
                    ...
                    {branchesError ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            <div style={{ color: '#b00020' }}>No se pudieron cargar las sucursales: {branchesError}</div>
                            <div style={{ display: 'flex', gap: 8 }}>
                                <button className={styles.clearBtn} onClick={() => fetchBranches()}>Reintentar sucursales</button>
                                <button className={styles.clearBtn} onClick={() => setBranchesError(null)}>Ocultar</button>
                            </div>
                        </div>
                    ) : (
                        <select
                        ...
                        >
                            ...
                            {branches.map((b) => (
                                <option key={b.codSucursal} value={b.codSucursal}>
                                    {b.nombre} {b.calle ? `- ${b.calle}` : ""}{b.altura ? ` ${b.altura}` : ""}
                                </option>
                            ))}
                        </select>
                    )}
