      ...
      try {
        ...
        const parseJsonOrThrow = async (res: Response, name = "resource") => {
          ...
          if (!res.ok) {
            ...
            throw new Error(
              `${name} fetch failed: ${res.status} ${res.statusText} - ${text.substring(0, 300)}`,
            );
          }
          ...
          throw new Error(
            `${name} did not return JSON. Response body: ${text.substring(0, 500)}`,
          );
        };
        ...
        const sucursalesData = await parseJsonOrThrow(
          sucursalesRes,
          "sucursales",
        );
        ...
        const turnosData =
          turnosDataRaw && turnosDataRaw.data
            ? turnosDataRaw.data
            : turnosDataRaw;
        ...
        console.error(error);
        toast.error(
          "No se pudieron cargar los datos. Revisa la consola y que el backend esté corriendo.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const revenueByBranch = useMemo(() => {
    ...
    for (const t of turnos) {
      try {
        ...
        const codSucursal =
          barberoToSucursal.get(t.codBarbero) || "_SIN_SUCURSAL_";
    ...
    return result;
  }, [turnos, barberoToSucursal, month, year, sucursales]);
    ...
    <div className={styles.mainTurnos}>
      ...
      <div className={localStyles.tableWrapper}>
        <table className={localStyles.rentTable}>
          ...
          <tbody>
            {Array.from(revenueByBranch.entries()).map(
              ([codSucursal, total]) => (
                <tr key={codSucursal}>
                  <td className={localStyles.tdCell}>
                    {codSucursal === "_SIN_SUCURSAL_"
                      ? "Sin sucursal asignada"
                      : sucursalNames.get(codSucursal) || codSucursal}
                  </td>
