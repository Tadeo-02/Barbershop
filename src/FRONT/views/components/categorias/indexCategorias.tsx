...
const IndexCategorias = () => {
    ...
    const handleDelete = async (codCategoria: number) => {
        const confirmed = window.confirm("¿Estás seguro de que querés borrar esta categoría?");
        if (!confirmed) return;
        ...
        try {
            ...
            if (response.ok) {
                ...
                setCategorias(categorias.filter((categoria) => categoria.codCategoria !== codCategoria));
