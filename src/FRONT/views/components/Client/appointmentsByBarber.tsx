...
const AppointmentsByBarber = () => {
  ...
  const generateSlots = (
    ...
    for (
      let t = totalMinutesStart;
      t + stepMinutes <= totalMinutesEnd;
      t += stepMinutes
    ) {
  ...
  const refreshAppointments = async (fecha?: string) => {
    if (!barberId) return;
    const targetDateRaw =
      fecha ?? selectedDate ?? new Date().toISOString().substring(0, 10);
    ...
    try {
      ...
      const occupied = list.filter((a) => {
        try {
          ...
          if (fechaTurnoRaw) {
            const simpleDate =
              String(fechaTurnoRaw).match(/^\d{4}-\d{2}-\d{2}$/);
            if (simpleDate) {
          ...
          const isCancelled = !!(
            a.fechaCancelacion && String(a.fechaCancelacion).trim() !== ""
          );
      ...
      for (const o of occupied) {
        if (o.horaDesde) {
          try {
            const dt = new Date(o.horaDesde);
            if (!isNaN(dt.getTime())) {
              occupiedSet.add(dt.toTimeString().substring(0, 5));
              continue;
            }
          } catch {}
        }
        if (o.hora) {
          ...
          if (match) {
            occupiedSet.add(match[1].padStart(5, "0"));
            continue;
          }
        }
        if (o.hora_desde) {
          try {
            const d2 = new Date(o.hora_desde);
            if (!isNaN(d2.getTime())) {
              occupiedSet.add(d2.toTimeString().substring(0, 5));
              continue;
            }
          } catch {}
        }
      ...
      const availableSlots = generated.filter(
        (s: { hora: string }) => !occupiedSet.has(s.hora)
      );
      const availAsAppointments = availableSlots.map(
        (h: { hora: string }, i: number) => ({
          id: 1000 + i,
          fecha: targetDate,
          horaDesde: h.hora,
          hora: h.hora,
      ...
      const sortedAvail = availAsAppointments.sort((a: any, b: any) =>
        (a.hora || a.horaDesde).localeCompare(b.hora || b.horaDesde)
      );
      setAppointments(sortedAvail);
    } catch (err) {
      console.error("Error fetching appointments:", err);
      const availAsAppointments = generated.map(
        (h: { hora: string }, i: number) => ({
          id: 1000 + i,
          fecha: targetDate,
          horaDesde: h.hora,
  ...
  const handleReserveAppointment = (appointmentId: number) => {
    // Para el manual - mostrar toast de usuario vetado
    toast.error("Usuario VETADO, no puede solicitar turnos", {
      duration: 4000,
      position: "top-center",
      style: {
          background: "#ef4444",
      ...
      maxWidth: "300px", // Agregar ancho máximo
    ...
    return; // Terminar aquí para mostrar solo el toast
  };
    ...
    <div className={styles.appointmentsContainer}>
      {loadingBarber ? (
        ...
        <div className={styles.barberInfo}>
          ...
          {barbero.telefono && (
            <div>
              <strong>Teléfono:</strong> {barbero.telefono}
            </div>
          )}
          {barbero.email && (
            <div>
              <strong>Email:</strong> {barbero.email}
            </div>
          )}
        </div>
      ) : (
        <div className={styles.emptyState}>
          No se encontró información del barbero.
        </div>
      )}
      <h3>
      ...
      <div className={styles.appointmentList}>
        {appointments.length === 0 ? (
          <div className={styles.emptyState}>
            No hay turnos disponibles para este barbero.
          </div>
        ) : (
          <>
            <div className={styles.hoursGrid}>
              {appointments
                ...
                .map((appointment) => (
                  <div
                    key={appointment.id}
                    className={`${styles.hourCard} ${styles.hourAvailable}`}
                  >
                    <div>
                      <div className={styles.hourTime}>
                        {appointment.hora ?? appointment.horaDesde}
                      </div>
                    ...
                    >
                      ...
                      {formatDateToYYYYMMDD(selectedDate) < todayYYYYMMDD ? (
                        ...
                        <button
                          ...
                          onClick={() =>
                            handleReserveAppointment(appointment.id)
                          }
                        >
                          Reservar
                        </button>
            ...
            {appointments.length > perPage && (
              <div className={styles.pagination}>
                ...
                {Array.from(
                  { length: Math.ceil(appointments.length / perPage) },
                  (_, i) => (
                    <button
                      key={i}
                      className={`${styles.pageButton} ${
                        page === i + 1 ? styles.pageActive : ""
                      }`}
                      onClick={() => setPage(i + 1)}
                    >
                      {i + 1}
                ...
                <button
                  ...
                  onClick={() =>
                    setPage((p) =>
                      Math.min(Math.ceil(appointments.length / perPage), p + 1)
                    )
                  }
                  disabled={page === Math.ceil(appointments.length / perPage)}
                >
                  Next
                </button>
              </div>
