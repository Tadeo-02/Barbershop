  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/appointments/${appointment.codTurno}/cancel`,
        {
          method: "PUT",
      ...
      if (response.ok) {
        alert(message || "Fecha de cancelación registrada.");
        navigate("/indexAppointments");
      } else {
        alert(message || "Error al registrar la cancelación.");
      }
    } catch (error) {
        ...
        <div className={styles.appointmentDetails}>
          ...
          {appointment.precioTurno !== undefined && (
            <span className={styles.appointmentTime}>
              Precio: ${appointment.precioTurno}
            </span>
          )}
        </div>
      </div>
