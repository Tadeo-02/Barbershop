...
// ============================================================
// Template de Factura Electrónica (con datos ARCA)
// ============================================================

/** Datos necesarios para generar el PDF de factura */
...
/**
 * Genera un PDF de factura electrónica y lo devuelve como Buffer.
 */
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.fontSize(10).font("Helvetica").fillColor("#333333")
                .text(`N° ${formatVoucherNumber(data.puntoDeVenta, data.voucherNumber)}`, rightX, 34, { width: rightW, lineBreak: false });
            doc.fontSize(9).fillColor("#555555")
                .text(`Fecha de emisión: ${formatAfipDate(data.fechaEmision)}`, rightX, 52, { width: rightW, lineBreak: false });
            doc.fontSize(8).fillColor("#666666")
                .text(`Punto de venta: ${String(data.puntoDeVenta).padStart(4, "0")}`, rightX, 68, { width: rightW, lineBreak: false });
            ...
            const col1W = Math.floor(CW * 0.65);
            ...
            const caeDataY = Y + 30;
            const caeCol1X = MX + 10;
            const caeCol2X = HALF + 10;

            doc.fontSize(8).font("Helvetica").fillColor("#555555")
                .text("CAE N°:", caeCol1X, caeDataY, { lineBreak: false });
            doc.fillColor("#111111").font("Helvetica-Bold")
                .text(data.cae, caeCol1X + 52, caeDataY, { width: HALF - caeCol1X - 62, lineBreak: false });

            doc.fillColor("#555555").font("Helvetica")
                .text("Vto. CAE:", caeCol2X, caeDataY, { lineBreak: false });
            doc.fillColor("#111111").font("Helvetica-Bold")
                .text(formatAfipDate(data.caeFchVto), caeCol2X + 58, caeDataY, { lineBreak: false });

            doc.strokeColor("#cccccc").lineWidth(0.5)
                .moveTo(HALF, Y + 20).lineTo(HALF, Y + CAE_H - 8).stroke();
